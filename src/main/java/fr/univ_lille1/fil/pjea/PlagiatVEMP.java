...
import fr.univ_lille1.fil.pjea.comparators.NaiveSuccessivesTokensFileComparator;
		...
		for (int i=0; i<files.length; i++) {
			for (int j=i+1; j<files.length; j++) {
				try {
					...
					double result = new NaiveSuccessivesTokensFileComparator(files[i], files[j]).computeDifference();
